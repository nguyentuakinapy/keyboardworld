...
public interface OrderDAO extends JpaRepository<Order, Integer> {
	...
	@Query("SELECT new com.asmkbw.entity.NewOD(o.orderID, p.productID, pd.thumbNail, p.name, pd.color, od.quantity, pd.price, "
			+ "a.addRessDetail, a.ward, a.district, a.city, a.phone) " + "FROM Order o "
			+ "JOIN o.user u " + "JOIN o.orderDetails od " + "JOIN od.productDetail pd " + "JOIN pd.product p "
			+ "JOIN u.addresses a " + "WHERE u.email = ?1 AND o.orderID  = ?2")
